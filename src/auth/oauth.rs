...
use crate::auth::{discover_endpoints, PkceChallenge, generate_state};
...
impl OAuthClient {
    pub async fn new(profile: Profile) -> Result<Self> {
        ...
        let (authorization_endpoint, token_endpoint) = if let Some(ref discovery_uri) = profile.discovery_uri {
            let discovery_doc = discover_endpoints(discovery_uri).await?;
            (discovery_doc.authorization_endpoint, discovery_doc.token_endpoint)
        } else {
            let auth_endpoint = profile.authorization_endpoint.as_ref()
                .ok_or_else(|| OidcError::Config("Missing authorization endpoint".to_string()))?;
            let token_endpoint = profile.token_endpoint.as_ref()
